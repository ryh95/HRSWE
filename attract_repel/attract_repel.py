## Attract-Repel code from https://github.com/nmrksic/attract-repel
import configparser
import pickle
...
from constants import ATTRACT_REPEL_DIR
...
class ExperimentRun:
    ...
    def __init__(self, config):
        ...
        type = self.config.get("data","emb_type")
        distributional_vectors = load_word_vectors(distributional_vectors_filepath,type)
        ...
        synonym_list = self.config.get('data','synonyms_list')
        antonym_list = self.config.get('data','antonyms_list')
    ...
    def attract_repel(self,evaluator=None):
        """
    ...
    def print_word_vectors(self,word_vectors, write_path):
        """
        ...
        if type == 'txt':
            ...
            with open(write_path, 'wb') as f:
                pickle.dump({key: numpy.round(word_vectors[key], decimals=6) for key in word_vectors}, f,
...
def load_word_vectors(file_destination,type='glove'):
    ...
    if type == 'glove':
        try:
            ...
            f = codecs.open(file_destination, 'r')
        ...
        word_dictionary = KeyedVectors.load_word2vec_format(file_destination, binary=True)
        # change to dict type, in order to be consistent
        word_dictionary = {w:word_dictionary[w] for w in word_dictionary.vocab.keys()}
    elif type == 'pickle':
        with open(file_destination, 'rb') as handle:
            word_dictionary = pickle.load(handle)
...
def run_experiment(config):
    ...
    results directory.
    config: object returned by configparser.RawConfigParser()
    """
    current_experiment = ExperimentRun(config)
...
def main():
    ...
    config = configparser.RawConfigParser()
    # todo: set synonyms_list and antonyms_list to python list
    try:
        config.read(config_filepath)
    except:
