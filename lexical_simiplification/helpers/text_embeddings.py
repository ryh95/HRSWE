...
from lexical_simiplification.helpers import io_helper as ioh
import codecs
...
class Word2VecEmbedding(object):
	...
	def load_embeddings(self, filepath, limit, language = 'en', print_loading = False, skip_first_line = False, min_one_letter = False, special_tokens = None, normalize = False):
		print('load word2vec...')
		self.word2vec = KeyedVectors.load_word2vec_format(filepath, limit=limit,binary=True)
		print('load finished')
		if normalize:
	...
	def most_similar_fast_cosine(self, embedding, target_lang, num = 1, without_first = False):
		words = [word for word,vec in self.word2vec.similar_by_vector(embedding,topn=num)]
		return words[1:] if without_first else words

class PickleEmbedding(object):
	...
	def load_embeddings(self, filepath, limit, language='en', print_loading=False, skip_first_line=False,
		...
		if normalize:
			self.emb_dict = {w: v/np.linalg.norm(v) for w,v in self.emb_dict.items()}
			self.emb = self.emb/np.linalg.norm(self.emb,axis=1)[:,np.newaxis]
		self.id2word = {id:w for id,w in enumerate(self.vocab)}

	def get_vector(self, lang, word):
		return self.emb_dict[word] if word in self.emb_dict else None

	def most_similar_fast_cosine(self, embedding, target_lang, num=1, without_first=False):
		dot = np.dot(embedding, np.transpose(self.emb))
		indices = np.argsort(dot)[-1 * (num + (1 if without_first else 0)):][::-1]
		words = [self.id2word[ind] for ind in indices]
		return words[1:] if without_first else words