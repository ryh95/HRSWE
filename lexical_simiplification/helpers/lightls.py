...
class LightLS(object):
	...
	def evaluate_lex_mturk_simplification(self,simplifications,candidates):
		# report accuracy
		acc,change = 0,0
		for simplification,candidate in zip(simplifications,candidates):
			if simplification in candidate:
				acc += 1
	...
	def try_simplify_token(self, tokens, index):
		...
		# 	print('target is upper or title or numeric, not simplify it.')
		# 	print(target)
		# 	return None
		...
		if complexity_target <= self.params["complexity_threshold"]:
			print('target is too simple, not simplify it.')
			return None

		# we're not "simplifying" stopwords
		if self.stopwords and target.lower() in self.stopwords:
			print('target is a stopword, not simplify it')
			return None
		...
		if tvec is not None:
			...
			if len(simpler_candidates) == 0:
				print('number of simpler candidates is 0, quit')
				return None
			...
			if simpler_candidates[best_candidate]["sim"] >= self.params["similarity_threshold"]:
				...
				print('the similarity of best candidate is too low, not simplify it')
				return None
		else:
			print('can not retrive the vector of target, quit')
			return None
